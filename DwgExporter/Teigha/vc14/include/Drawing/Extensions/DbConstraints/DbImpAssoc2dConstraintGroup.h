...
/*!DOM*/
typedef std::map<OdConstraintGroupNodeId, OdDbAssoc2dConstraintGroup::SolutionStatus> OdConstraintStatusMap;
...
{
  typedef OdDbAssoc2dConstraintGroup::SolutionStatus SolutionStatus;
public:
  /** \details
  ...
  */
  OdResult transformActionBy(OdDbAssocAction* thisAction, const OdGeMatrix3d& xform);
  ...
  */
  SolutionStatus solutionStatus(const OdDbAssoc2dConstraintGroup* pGroup, 
    bool bAlsoCheckForConstraints = true) const;

  /** \details
    Returns the evaluation status of a given constrain geometry.
  */
  SolutionStatus geometryStatus(const OdDbAssoc2dConstraintGroup* pGroup, 
    const OdConstrainedGeometry* pConsGeom) const;

  /** \details
    Returns the evaluation status of a given constraint.
  */
  SolutionStatus constraintStatus(const OdDbAssoc2dConstraintGroup* pGroup,
  ...
  */
  ...
  /** \details
  */
  OdResult addMoreObjectsToDeepClone(const OdDbAssocAction *pThisAction, OdDbIdMapping & idMap,
    OdDbObjectIdArray & additionalObjectsToClone) const;

  /** \details
    Moves the constraint geometry.
  */
  OdResult moveConstrainedGeometry(const OdDbFullSubentPath& subentPath,
  ...
  */
  virtual OdResult dwgInFields(OdDbAssocAction *pSelf, OdDbDwgFiler* pFiler);
...
protected:
  /*!DOM*/
  OdDbObjectId findGeometryCorrespodingToDimension(const OdDbAssocDependency* pDimDep) const;
  /*!DOM*/
  OdGePlane               m_plane;
  /*!DOM*/
...
private:
  ...
  /*!DOM*/
  OdConstrainedRigidSet* getRigidSet(OdDbObjectId geomDepId, bool createIfNotExists = false);
  /*!DOM*/
  ...
  /*!DOM*/
  static void updateDependentOnObjects(OdDbAssocAction *p2dConstraintGroup);
};
