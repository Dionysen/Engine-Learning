...
class OdTrVisMetafileParser
{
  public:
    ...
    struct VarState
    {
      ...
      const float *asFVec(float *out = 0, OdUInt32 count = 0) const;
    };
...
inline const float*        OdTrVisMetafileParser::VarState::asFVec(float *out, OdUInt32 count) const
{
  memcpy(out, m_pVar, sizeof(float) * count);
  return out;
}
...
inline const float*        OdTrVisMetafileParser::VarState::asFVec(float *out, OdUInt32 count) const { return  (const float*)m_pVar; }
