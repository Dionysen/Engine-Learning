...
* non-spline fit OdDb3dPolyline
    ...
    * path doesn't perpendicular to region plane at path base point. The path base point is point on path that closest to region center from set:
    All interscetion points of region plane and path in case when path end point lie on region plane.
    Path endpoints in case when both endpoints doesn't lie on region plane.
...
* 3 <= sides <= 64.
  ...
  */
  ...
  /** \details
    ...
    This method is implemented for Spatial modeler and partially for ModelerGeometry.
    ModelerGeometry implementation at almost all cases return eNotImplementedYet status.
  */
  virtual OdResult createSweptSolid ( OdDbEntity* pSweepEnt,
    OdDbEntity* pPathEnt,
    OdDbSweepOptions& sweepOptions );
  ...
  /** \details
    ...
    \remarks
    If at least one of the obtained triangle points
