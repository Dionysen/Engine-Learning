...
template <typename REAL> struct FontVariantHeader {
  uint32 flags;
  uint32 weight;
...
template <typename REAL> uint32 realTypeCode();

template <> inline uint32 realTypeCode<float>() { return 0x14u; }
template <> inline uint32 realTypeCode<double>() { return 0x18u; }
...
template <class STRING> uint32 paddedStringLength(const STRING &str) {
...
template <ReadFunction READ, typename REAL, template <typename> class LIST,
          class BYTE_ARRAY, class STRING>
...
#define ARTERY_FONT_DECODE_READ(target, len)                                   \
  {                                                                            \
    ...
    for (int _i = 0; _i < int(len); ++_i)                                      \
      checksum =                                                               \
          crc32Update(checksum, reinterpret_cast<const byte *>(target)[_i]);   \
  }
...
#define ARTERY_FONT_DECODE_READ_STRING(str, len)                               \
  ...
  // Read header
  {
    ...
    font.appendices =
        LIST<FontAppendix<BYTE_ARRAY, STRING>>(header.appendixCount);
  ...
  for (int i = 0; i < variantCount; ++i) {
    ...
    ARTERY_FONT_DECODE_READ((Glyph<REAL> *)variant.glyphs,
                            header.glyphCount * sizeof(Glyph<REAL>));
    ARTERY_FONT_DECODE_READ((KernPair<REAL> *)variant.kernPairs,
                            header.kernPairCount * sizeof(KernPair<REAL>));
  }
...
template <WriteFunction WRITE, typename REAL, template <typename> class LIST,
          class BYTE_ARRAY, class STRING>
bool encode(const ArteryFont<REAL, LIST, BYTE_ARRAY, STRING> &font,
            void *userData) {
  uint32 totalLength = 0;
  uint32 checksum = crc32Init();
...
#define ARTERY_FONT_ENCODE_WRITE(data, len)                                    \
  {                                                                            \
    ...
    for (int _i = 0; _i < int(len); ++_i)                                      \
      checksum =                                                               \
          crc32Update(checksum, reinterpret_cast<const byte *>(data)[_i]);     \
  }
#define ARTERY_FONT_ENCODE_REALIGN()                                           \
  {                                                                            \
    if (totalLength & 0x03u) {                                                 \
      uint32 _len = 0x04u - (totalLength & 0x03u);                             \
      ARTERY_FONT_ENCODE_WRITE(padding, _len);                                 \
    }                                                                          \
  }
#define ARTERY_FONT_ENCODE_WRITE_STRING(str)                                   \
  ...
  // Write header
  {
    ...
    header.metadataLength = (uint32)font.metadata.length();
    header.variantCount = variantCount = (int)font.variants.length();
    header.variantsLength = 0;
    header.imageCount = imageCount = (int)font.images.length();
    header.imagesLength = 0;
    header.appendixCount = appendixCount = (int)font.appendices.length();
    ...
    for (int i = 0; i < variantCount; ++i) {
      ...
      header.variantsLength +=
          (uint32)(variant.glyphs.length() * sizeof(Glyph<REAL>));
      header.variantsLength +=
          (uint32)(variant.kernPairs.length() * sizeof(KernPair<REAL>));
    }
    for (int i = 0; i < imageCount; ++i) {
      ...
      header.imagesLength +=
          internal::paddedLength((uint32)image.data.length());
    }
    for (int i = 0; i < appendixCount; ++i) {
      const FontAppendix<BYTE_ARRAY, STRING> &appendix = font.appendices[i];
      header.appendicesLength += sizeof(internal::AppendixHeader);
      header.appendicesLength +=
          internal::paddedStringLength(appendix.metadata);
      header.appendicesLength +=
          internal::paddedLength((uint32)appendix.data.length());
    }
  ...
  for (int i = 0; i < variantCount; ++i) {
    ...
    header.nameLength = (uint32)variant.name.length();
    header.metadataLength = (uint32)variant.metadata.length();
    header.glyphCount = (uint32)variant.glyphs.length();
    header.kernPairCount = (uint32)variant.kernPairs.length();
    ARTERY_FONT_ENCODE_WRITE(&header, sizeof(header));
    ARTERY_FONT_ENCODE_WRITE_STRING(variant.name);
    ARTERY_FONT_ENCODE_WRITE_STRING(variant.metadata);
    ARTERY_FONT_ENCODE_WRITE((const Glyph<REAL> *)variant.glyphs,
                             header.glyphCount * sizeof(Glyph<REAL>));
    ARTERY_FONT_ENCODE_WRITE((const KernPair<REAL> *)variant.kernPairs,
                             header.kernPairCount * sizeof(KernPair<REAL>));
  }
  // Write images
  for (int i = 0; i < imageCount; ++i) {
    ...
    memset(header.reserved, 0, sizeof(header.reserved));
    header.metadataLength = (uint32)image.metadata.length();
    header.dataLength = (uint32)image.data.length();
    ARTERY_FONT_ENCODE_WRITE(&header, sizeof(header));
    ARTERY_FONT_ENCODE_WRITE_STRING(image.metadata);
    ARTERY_FONT_ENCODE_WRITE((const unsigned char *)image.data,
                             header.dataLength);
    ARTERY_FONT_ENCODE_REALIGN();
  }
  // Write appendices
  for (int i = 0; i < appendixCount; ++i) {
    const FontAppendix<BYTE_ARRAY, STRING> &appendix = font.appendices[i];
    internal::AppendixHeader header;
    header.metadataLength = (uint32)appendix.metadata.length();
    header.dataLength = (uint32)appendix.data.length();
    ARTERY_FONT_ENCODE_WRITE(&header, sizeof(header));
    ARTERY_FONT_ENCODE_WRITE_STRING(appendix.metadata);
    ARTERY_FONT_ENCODE_WRITE((const unsigned char *)appendix.data,
                             header.dataLength);
    ARTERY_FONT_ENCODE_REALIGN();
  }
  // Write footer
