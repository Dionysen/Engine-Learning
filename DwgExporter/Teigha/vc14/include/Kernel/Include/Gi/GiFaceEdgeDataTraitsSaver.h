...
class OdGiFaceEdgeDataTraitsSaverBase
{
  ...
  protected:
    ...
    {
      std::map< OdDbStub*, bool >::iterator it = m_layersByBlock.find( layerId );
      if( it != m_layersByBlock.end() ) return it->second;
      OdGiDrawablePtr pDr = m_pDrawCtx->giContext().openDrawable( layerId );
      if( pDr.isNull() )
      {
        m_layersByBlock[ layerId ] = false;
        return false;
      }
      OdGiLayerTraitsTaker taker;
      OdUInt32 flgs = pDr->setAttributes( static_cast<OdGiLayerTraits*>(&taker) );
      m_layersByBlock[ layerId ] = GETBIT( flgs, OdGiLayerTraits::kByBlock );
      return GETBIT( flgs, OdGiLayerTraits::kByBlock );
    }
  ...
  protected:
    ...
    {
      m_defaultLayerId = m_pTraits->layer();
    }
    ...
    bool setLayer(OdDbStub **pLayers, int index)
    {
      ...
      {
        if( !isDefaultLayer( pLayers[index] ) || !m_defaultLayerId )
          m_pTraits->setLayer(m_curLayerId = pLayers[index]);
        else if( m_curLayerId != m_defaultLayerId )
        {
          m_pTraits->setLayer(m_curLayerId = m_defaultLayerId);
        }
        return true;
      }
      return false;
...
class OdGiFaceDataTraitsSaver : protected virtual OdGiFaceEdgeDataTraitsSaverBase
{
  ...
  public:
    OdGiFaceDataTraitsSaver(const OdGiFaceData *pFaceData, OdGiSubEntityTraits *pTraits, OdGiConveyorContext *pDrawCtx,
                            OdGsView::RenderMode renderMode, bool bForEdge = false, bool ignoreFaceVisibilities = false)
      : OdGiFaceEdgeDataTraitsSaverBase(pTraits, pDrawCtx, renderMode), m_pFaceData(pFaceData)
    {
      if (pFaceData && pTraits)
      {
        ...
        if (pFaceData->visibility() && !ignoreFaceVisibilities)
...
class OdGiFaceEdgeDataTraitsSaver : public OdGiFaceDataTraitsSaver, public OdGiEdgeDataTraitsSaver
{
  public:
    OdGiFaceEdgeDataTraitsSaver(const OdGiFaceData *pFaceData, const OdGiEdgeData *pEdgeData,
      ...
      , OdGiFaceDataTraitsSaver(pFaceData, pTraits, pDrawCtx, renderMode, true, true)
