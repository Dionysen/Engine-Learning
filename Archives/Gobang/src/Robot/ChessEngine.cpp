...
#define MIN_SCORE (-10000000)
    ...
    int evaluatePoint(char board[BOARD_WIDTH][BOARD_WIDTH], Position p)
    {
        ...
        for (i = p.x - min(min(p.x, p.y), 5), j = p.y - min(min(p.x, p.y), 5);
             i < (unsigned int)min(BOARD_WIDTH, p.x + 6) && j < (unsigned int)min(BOARD_WIDTH, p.y + 6); i++, j++)
        {
            if (i != p.x)
            {
        ...
        for (i = p.x + min(min(p.y, BOARD_WIDTH - 1 - p.x), 5), j = p.y - min(min(p.y, BOARD_WIDTH - 1 - p.x), 5);
             i >= (unsigned int)max(0, p.x - 5) && i < BOARD_WIDTH && j < (unsigned int)min(BOARD_WIDTH, p.y + 6); i--, j++)
        {
            if (i != p.x)
            {
