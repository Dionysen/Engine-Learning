...
protected:
  ...
  */
  void onFinalRelease();
public:
  class GsViewUpdateEvent
  {
    OdGsBaseModule* m_pModule;
    OdGsView* m_pView;
    int m_flags;
  public:
    GsViewUpdateEvent(OdGsBaseModule* pModule, OdGsView* pView, int flags)
      : m_pModule(pModule), m_pView(pView), m_flags(flags)
    {
      if (pModule)
        pModule->fire_viewToBeUpdated(pView, flags);
    }
    ~GsViewUpdateEvent()
    {
      if (m_pModule)
        m_pModule->fire_viewWasUpdated(m_pView, m_flags);
    }
  };
public:
