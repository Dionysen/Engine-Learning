...
#define OdUnorderedSet stdext::hash_set
namespace std 
{
...
template <class T> struct OdHashSet_PtrHasher : OdUnorderedSet_DefaultHasher<OdIntPtr> {
  enum { bucket_size = 4, min_buckets = 8 };
  size_t operator()(const T* key) const { return OdUnorderedSet_DefaultHasher<OdIntPtr>::operator()((OdIntPtr)key); }
};
