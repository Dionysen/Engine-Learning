...
template <typename REAL> struct KernPair {
...
template <class BYTE_ARRAY, class STRING> struct FontImage {
  uint32 flags;
...
template <class BYTE_ARRAY, class STRING> struct FontAppendix {
  STRING metadata;
  BYTE_ARRAY data;
};

template <typename REAL, template <typename> class LIST, class BYTE_ARRAY,
          class STRING>
struct ArteryFont {
