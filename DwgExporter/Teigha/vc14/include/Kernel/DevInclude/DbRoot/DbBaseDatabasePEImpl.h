...
public:
  ...
  TD_USING(OdDbBaseDatabasePE::openObject);
  virtual OdRxObjectPtr openObject(OdDbStub* pId, bool bForWrite);
...
public:
  ...
  virtual OdResult getPlotType(const OdRxObject*, OdDbBaseLayoutPE::PlotType&) const;
  virtual OdResult getPlotPaperUnits(const OdRxObject*, OdDbBaseLayoutPE::PlotPaperUnits&) const;
  virtual OdResult getStdScaleType(const OdRxObject*, OdDbBaseLayoutPE::StdScaleType&) const;
};

class DBROOT_EXPORT OdSetBasePlotSettingsPEImpl : public OdDbSetBasePlotSettingsPE
{
public:
  ODRX_DECLARE_MEMBERS(OdSetBasePlotSettingsPEImpl);
