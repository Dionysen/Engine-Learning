...
namespace Oda { // shortened Drawings identifiers
  ...
  inline OdString getDbCtxName(OdEdCommandContext* ctx) {
    ...
    OdExtDbUtils::DbType dbType = OdExtDbUtils::getProduct(db);
    switch (dbType)
    {
    case OdExtDbUtils::kDbDwg: return "DwgCtx";
    case OdExtDbUtils::kDbDgn: return "DgnCtx";
    //case OdExtDbUtils::kDbPrc: return "PrcCtx";// *AutoUpdateReactor commands not implemented
    //case OdExtDbUtils::kDbBim: return "BimCtx";// DNA: AutoUpdateReactor will not work with BIM because OdBmDatabaseReactor has no goodby() callback method
    }
    ODA_FAIL_ONCE();
    return OdString();
  }
