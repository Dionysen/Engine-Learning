...
class OdTrVisRenderDataInvoke : public OdTrVisRenderDataInvokeHid
{
  public:
    ...
    virtual bool incomeHlMarkers(OdTrVisStateBranchId /*hlBranch*/) { return true; }
    /** \details
    Processes visibilities markers
    \param vsBranch [out] Id of visibility branch
    \returns true if next data should be processed too
    */
    virtual bool incomeVsMarkers( OdTrVisStateBranchId /*vsBranch*/ ) { return true; }
...
class OdTrVisRenderDataInvokeRedirect : public OdTrVisRenderDataInvoke
{
  ...
  public:
    ...
    virtual bool incomeHlMarkers(OdTrVisStateBranchId hlBranch)
    {
      if (hasRedirection())
        return getRedirection()->incomeHlMarkers(hlBranch);
      return defRv();
    }
    virtual bool incomeVsMarkers( OdTrVisStateBranchId vsBranch )
    {
      if( hasRedirection() )
        return getRedirection()->incomeVsMarkers( vsBranch );
      return defRv();
    }
...
class OdTrVisRenderDataInvokeBranch : public OdTrVisRenderDataInvoke
{
  ...
  public:
    ...
    { RvFlag rv;
      if (hasRedirectionA())
        rv.add(getRedirectionA()->incomeHlMarkers(hlBranch));
      if (hasRedirectionB() && rv.state())
        rv.add(getRedirectionB()->incomeHlMarkers(hlBranch));
      return rv.generateRv() ? rv.state() : defRv();
    }
    virtual bool incomeVsMarkers( OdTrVisStateBranchId vsBranch )
    {
      RvFlag rv;
      if( hasRedirectionA() )
        rv.add( getRedirectionA()->incomeVsMarkers( vsBranch ) );
      if( hasRedirectionB() && rv.state() )
        rv.add( getRedirectionB()->incomeVsMarkers( vsBranch ) );
      return rv.generateRv() ? rv.state() : defRv();
    }
    virtual bool incomeDrawOrder(const double* pDrawOrder)
    { RvFlag rv;
