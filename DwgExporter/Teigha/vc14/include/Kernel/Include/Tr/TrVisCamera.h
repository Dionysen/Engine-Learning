...
public:
  ...
  void setContext(const OdTrVisSurfaceDef *pSurface, const OdTrVisViewportShadingDef *pViewportShading,
                  const OdTrVisViewportClipDef *pViewportClip, const OdTrVisExtentsDef *pExtents)
  { m_pSurfaceDef = pSurface; m_pViewportShadingDef = pViewportShading; m_pViewportClipDef = pViewportClip; m_pExtentsDef = pExtents; }
  ...
  const OdGePoint3d &viewPosition() const { return m_viewParams.m_viewBasis.origin(); }
  void setViewPosition( const OdGePoint3d& pt )
  {
    m_viewParams.m_viewBasis.setOrigin(pt);
    setModified();
  }

  const OdGeVector3d &viewBasisX() const { return m_viewParams.m_viewBasis.xAxis(); }
  void setViewBasisX(const OdGeVector3d &xBasis)
  {
    m_viewParams.m_viewBasis.setXAxis(xBasis);
    setModified();
  }
  const OdGeVector3d &viewBasisY() const { return m_viewParams.m_viewBasis.yAxis(); }
  void setViewBasisY(const OdGeVector3d &yBasis)
  {
    m_viewParams.m_viewBasis.setYAxis(yBasis);
    setModified();
  }
  const OdGeVector3d &viewBasisZ() const { return m_viewParams.m_viewBasis.zAxis(); }
  void setViewBasisZ(const OdGeVector3d &zBasis)
  {
    m_viewParams.m_viewBasis.setZAxis(zBasis);
    setModified();
  }
