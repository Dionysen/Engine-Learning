...
//  bool isCancelling() const;
  ...
  */
  ...
  /** \details
    ...
    \param idMap [in/out] Object ID mapping object.
    \param pOwner [in]  Pointer to the owner object.

    \remarks
    Returns a SmartPointer to the newly created clone,
    and adds a record to the specified ID mapping object.
    ...
    \sa
    <link OdDbObject::subDeepClone@OdDbIdMapping&@OdDbObject*@bool@const, subDeepClone()>
    <link OdDbObject::wblockClone@OdDbIdMapping&@OdDbObject*@bool@const, wblockClone()>
    <link OdDbObject::appendToOwner@OdDbIdPair&@OdDbObject*@OdDbIdMapping&, appendToOwner()>
  */
  ...
  /** \details
    ...
    \sa
    <link OdDbObject::subWblockClone@OdDbIdMapping&@OdDbObject*@bool@const, subWblockClone()>
    <link OdDbObject::deepClone@OdDbIdMapping&@OdDbObject*@bool@const, deepClone()>
    <link OdDbObject::appendToOwner@OdDbIdPair&@OdDbObject*@OdDbIdMapping&, appendToOwner()>
  */
  ...
  /** \details
    ...
    during the deep or wblock clone operation.

    \param idPair [in]  Original-to-clone ID mapping record for this clone.
    \param pOwnerObject [in]  Pointer to the owner object.
    \param idMap [in/out] ID mapping object of the cloning operation in progress.

    \remarks
    This callback is called before the IDs are translated in the target database
    so the implementer should use only methods certainly not traversing hierarchy
    in the target database. To follow clone's references properly they should be
    translated using the ID mapping object.

    The default implementation appends this object to OdDbDictionary or throws
    the eInvalidOwnerObject exception if the owner object is not a kind of dictionary.
  ...
  */
  ...
  /** \details
    ...
    * Returns NULL, setting replaceId for a database -resident (DBRO) replacement object.
    ...
    Custom objects can decompose themselves into other objects, adding additional XData as required. The Drawings 
