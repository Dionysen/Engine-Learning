...
class OdTrVisVisualStyle
{
  ...
  public:
    ...
    void clear() { m_props.setPhysicalLength(0); }
...
void OdTrVisVisualStyle::computeRenderMode(OdTrVisRenderMode &pRM, bool bBestMatch) const
{
  ...
  if (bBestMatch)
  {
    ...
    if (pRM.is_2d() && property(OdGiVisualStyleProperties::kFaceLightingModel).getInt() > OdGiVisualStyleProperties::kInvisible)
      pRM.setDepthBufferEnabled(true), pRM.setStencilBufferEnabled(true); // #VIS-654
  }
}
