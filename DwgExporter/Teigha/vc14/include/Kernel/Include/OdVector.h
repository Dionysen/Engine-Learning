...
, m_logicalLength(vec.m_logicalLength), m_growLength(vec.m_growLength)
{
  if(m_physicalLength > 0)
  {
    ...
    A::constructn(m_pData, vec.m_pData, m_logicalLength);
  }
}
...
inline OdVector<T, A, Mm>& OdVector<T, A, Mm>::operator=(const OdVector<T, A, Mm>& vec)
{
  if(this != &vec)
  {
    ...
    if (vec.m_logicalLength > 0)
    {
      ...
      A::constructn(m_pData, vec.m_pData, vec.m_logicalLength);
...
inline void OdVector<T, A, Mm>::insert(VEC_ITERATOR before, VEC_CONST_ITERATOR first, VEC_CONST_ITERATOR afterLast)
{
  ...
  if(index <= m_logicalLength && afterLast >= first)
  {
    if(afterLast > first)
    {
      ...
      {
        A::constructn(m_pData + oldLogicalLength, numElem);
        m_logicalLength = newLogicalLength;
      ...
      {
        VEC_SIZE_TYPE newPhysicalLength = calcPhysicalLength(newLogicalLength);
        T* pNewData = allocate(newPhysicalLength);
        A::constructn(pNewData, m_pData, index);
        A::constructn(pNewData + index, first, numElem);
        A::constructn(pNewData + index + numElem, m_pData + index, m_logicalLength - index);
        release();
        m_pData = pNewData;
        m_physicalLength = newPhysicalLength;
        m_logicalLength = newLogicalLength;
...
inline VEC_ITERATOR OdVector<T, A, Mm>::insert(VEC_ITERATOR before, VEC_SIZE_TYPE numElem, const T& value)
{
  ...
  if (index == oldLogicalLength)
  {
    ...
    else
    {
      ...
      A::constructn(m_pData + oldLogicalLength, numElem, valueCopy);
    }
    m_logicalLength = newLogicalLength;
  }
...
inline void OdVector<T, A, Mm>::resize(VEC_SIZE_TYPE logicalLength, const T& value)
{
  ...
  if(lengthDiff > 0)
  {
    ...
    A::constructn(m_pData + oldLogicalLength, lengthDiff, valueCopy);
  }
...
inline void OdVector<T, A, Mm>::push_back(const T& value)
{
  ...
  {
    A::construct(m_pData + m_logicalLength, value);
  }
  else
  {
    const T valueCopy(value);
    reallocate(m_logicalLength + 1, true);
    A::construct(m_pData + m_logicalLength, valueCopy);
  }
  ++m_logicalLength;
}
