...
public:  
  ...
  */
  ...
  /** \details
  Creates a view-dependent representation which consists of
  visible and hidden curves of input objects
  ...
  \param visCurves [in/out] edgeVisitor receives edges as odgecurves and it's visible/hidden segments
  ...
  */
  virtual OdResult booleanOper(OdDbDatabase* database, OdDb::BoolOperType operation, OdSmartPtr<OdModelerGeometry> otherRegion) = 0;
...
* kTrimBoth
  ...
  */  
  ...
  /** \details
   This function generates a data to show  ViewSection
  /// Sectionable entities include Db3dSolid, DbSurface, DbBody, and DbRegion.
  /// Returns eOk if successful. Returns an error status if the function fails to open the objector or if the plane and the solid do not intersect.
  ...
  \param edgeVis  [in] specifying the visibility of tangent, interference and other some tipes of edges

  \param backgroundEnts   [out] Array of resuts entities after señtion array of entities pEnts.
  \param intHatchEnt      [out] Array of planes section of resuts entities backgroundEnts
  \param intBoundaryEnts  [out] Array of intHatchEnt boundaries
  \param curveVisibilityEdges [out] Array of visibility edges to draw view section
  ...
  */
  ...
  /** \details
   ...
   \param arrFacesColors    [in] "true" an particular index means that face has colors
   \param arrFacesMaterials [in] "true" an particular index means that face has material
   \param pEntity           [out] created entity
   \returns
   * eOk - if entity created successfully.
   * eInvalidContext - if it is not possible to create a OdDb3dSolid.
   * eInvalidInput   - if any other error.
...
/** \details
    This template class is a specialization of the OdArray class for OdModelerGeometryPtr object pointers.
*/
