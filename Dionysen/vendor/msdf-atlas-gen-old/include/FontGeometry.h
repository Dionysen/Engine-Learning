...
public:
  ...
  public:
    GlyphRange();
    GlyphRange(const std::vector<GlyphGeometry> *glyphs, size_t rangeStart,
               size_t rangeEnd);
    size_t size() const;
    bool empty() const;
  ...
  /// Loads kerning pairs for all glyphs that are currently present, returns the
  /// number of loaded kerning pairs
  ...
  /// Finds a glyph by glyph index or Unicode codepoint, returns null if not
  /// found
  const GlyphGeometry *getGlyph(msdfgen::GlyphIndex index) const;
  const GlyphGeometry *getGlyph(unicode_t codepoint) const;
  /// Outputs the advance between two glyphs with kerning taken into
  /// consideration, returns false on failure
  bool getAdvance(double &advance, msdfgen::GlyphIndex index1,
                  msdfgen::GlyphIndex index2) const;
  bool getAdvance(double &advance, unicode_t codepoint1,
                  unicode_t codepoint2) const;
  /// Returns the complete mapping of kerning pairs (by glyph indices) and their
  /// respective advance values
