...
public:
  ...
  /** \details
    ...
    \param pField [in]  field defining the requested property,
                        containing at least "SheetSetPropertyName" data item
    \param pDb [in]  database

    Returns
        eOk  if the property was successfully retrieved to 'propValue',
        eInvalidInput  if 'pField' does not contain data defining the requested property,
        eWrongDatabase  if sheet set (OdSmDatabase or OdSmSheetSet) could not be found using 'pDb'.
  */
...
/** \details
  ...
  \param sString      [in] The string to split.
  \param aSubStrings [out] An array of strings, consisting of individual fields and portions of plain text.
 */
...
/** \details
  ...
  \param sString  [in] The string to parse.
  \param sName   [out] The name of the AcVar.
  \param sFormat [out] The format of the AcVar.
  \returns true - if specified string contains AcVar, false - otherwise.
 */
