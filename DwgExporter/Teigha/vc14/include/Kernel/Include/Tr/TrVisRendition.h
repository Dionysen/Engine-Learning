...
class OdTrVisRendition : public OdRxObject
{
  public:
    ...
    enum DisplayIdCodes
    {
      ...
      kDisplayCodeFade        = 5, // Execute fading procedure
      kDisplayCodeUnfade      = 6, // Execute unfading procedure
      kDisplayCodeHlMarkers   = 7, // Setup highlight markers
      kDisplayCodeDrawOrder   = 8, // Provide drawing order for upcoming geometry
      kDisplayCodeMultiOrder  = 9, // Upcoming geometry contain implicated drawing order
      kDisplayCodeVsMarkers   = 10,// Setup visibility markers
      kDisplayCodeSelectionID = 11,// Entity id for selection
    ...
    enum RenditionFlags
    {
      ...
      kSupportGPUSelection        = (1 << 8), // isSupportGPUSelection
    };
  public:
  ...
  public:
    ...
    /** \details
    Checks GPU selection support

    \returns true if GPU selection is supported
    */
    ...
    virtual void onStateBranchAdded(OdTrVisStateBranchId hlBranchId, const OdTrVisStateBranchDef &pDef) = 0;
    ...
    virtual void onStateBranchDeleted(OdTrVisStateBranchId hlBranchId) = 0;
    ...
    virtual void onStateBranchPropertyAttached(OdTrVisStateBranchId hlBranchId, const OdTrVisPropertyDef &propDef) = 0;
    // Called to detach property from state branch.
    virtual void onStateBranchPropertyDetached(OdTrVisStateBranchId hlBranchId, OdTrVisPropertyDef::PropertyType propType) = 0;
    ...
    \param pDevice [in] Processing device
    ...
    */
    virtual void getSelectedObject(OdTrVisRenderClient *pDevice, OdTrVisViewportId id, OdUInt32 x, OdUInt32 y, OdArray<OdTrVisId>& outIDs, OdTrVisGsMarker* outMarker = NULL) {}
};
...
class OdTrVisRenditionDef : public OdTrVisRendition
{
  ...
  public:
    ...
    // HlBranch-related properties.
    // Called to attach property to state branch.
    virtual void onStateBranchPropertyAttached(OdTrVisStateBranchId /*hlBranchId*/, const OdTrVisPropertyDef & /*propDef*/) { }
    // Called to detach property from state branch.
    virtual void onStateBranchPropertyDetached(OdTrVisStateBranchId /*hlBranchId*/, OdTrVisPropertyDef::PropertyType /*propType*/) { }
    // Viewport-related properties.
    // Called to attach property to viewport.
...
class OdTrVisRenditionRedir : public OdTrVisRendition
{
  ...
  public:
    ...
    {
      if (hasRedirection())
        getRedirection()->onStateBranchAdded(hlBranchId, pDef);
    }
    // Called when exist state branch modified.
    virtual void onStateBranchModified(OdTrVisStateBranchId hlBranchId, const OdTrVisStateBranchDef &pDef,
                                    bool bChildsModified, bool bMarkersModified)
    {
      if (hasRedirection())
        getRedirection()->onStateBranchModified(hlBranchId, pDef, bChildsModified, bMarkersModified);
    }
    // Called when exist state branch deleted.
    virtual void onStateBranchDeleted(OdTrVisStateBranchId hlBranchId)
    {
      if (hasRedirection())
        getRedirection()->onStateBranchDeleted(hlBranchId);
    }
    ...
    {
      if (hasRedirection())
        getRedirection()->onStateBranchPropertyAttached(hlBranchId, propDef);
    }
    // Called to detach property from state branch.
    virtual void onStateBranchPropertyDetached(OdTrVisStateBranchId hlBranchId, OdTrVisPropertyDef::PropertyType propType)
    {
      if (hasRedirection())
        getRedirection()->onStateBranchPropertyDetached(hlBranchId, propType);
    }
    // Viewport-related properties.
    // Called to attach property to viewport.
    virtual void onViewportPropertyAttached(OdTrVisViewportId viewportId, const OdTrVisPropertyDef &propDef)
