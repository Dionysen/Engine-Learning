...
class OdTrVisRenditionBranching : public OdTrVisRendition
{
  ...
  public:
    ...
    {
      if (hasRenditionA())
        getRenditionA()->onDeviceUpdateEnd(pDevice);
      if (hasRenditionB())
        getRenditionB()->onDeviceUpdateEnd(pDevice);
    }
  ...
  public:
    ...
    {
      if (hasRenditionA() && getRenditionA()->queryMetafileStream(displayId, pStream))
        return true;
      if (hasRenditionB()) 
        return getRenditionB()->queryMetafileStream(m_displayIdMapping.remap(displayId), pStream);
      return false;
    }
                                            ...
                                            OdTrVisGeomType geomType = OdTrVisGeomType_Default, OdUInt32 shadingFlags = 0) const
    {
      if (hasRenditionA())
      {
        const OdTrVisProgramId prgId = getRenditionA()->queryProgramId(mode, vsId, geomType, shadingFlags);
        if (prgId != kTrVisNegativeId)
          return true;
      }
      if (hasRenditionB())
        return getRenditionB()->queryProgramId(mode, vsId, geomType, shadingFlags);
    ...
    {
      if (hasRenditionA())
        getRenditionA()->onStateBranchAdded(hlBranchId, pDef);
      if (hasRenditionB())
        getRenditionB()->onStateBranchAdded(hlBranchId, pDef);
    }
    ...
    {
      if (hasRenditionA())
        getRenditionA()->onStateBranchModified(hlBranchId, pDef, bChildsModified, bMarkersModified);
      if (hasRenditionB())
        getRenditionB()->onStateBranchModified(hlBranchId, pDef, bChildsModified, bMarkersModified);
    }
    // Called when exist state branch deleted.
    virtual void onStateBranchDeleted(OdTrVisStateBranchId hlBranchId)
    {
      if (hasRenditionA())
        getRenditionA()->onStateBranchDeleted(hlBranchId);
      if (hasRenditionB())
        getRenditionB()->onStateBranchDeleted(hlBranchId);
    }
    ...
    {
      if (hasRenditionA())
        getRenditionA()->onStateBranchPropertyAttached(hlBranchId, propDef);
      if (hasRenditionB())
        getRenditionB()->onStateBranchPropertyAttached(hlBranchId, propDef);
    }
    ...
    {
      if (hasRenditionA())
        getRenditionA()->onStateBranchPropertyDetached(hlBranchId, propType);
      if (hasRenditionB())
        getRenditionB()->onStateBranchPropertyDetached(hlBranchId, propType);
    }
