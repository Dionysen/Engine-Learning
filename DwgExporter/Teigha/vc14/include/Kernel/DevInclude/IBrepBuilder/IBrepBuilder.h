...
public:
  ...
  // Set initial tag value(it is used for faces and edges)
  virtual void setInitialTag(OdUInt32 indTag = 0) = 0;
  // Set tag value(it is used for faces and edges) MODE: kSetTagMode; 
  // Use setTag just after addEdge and addFace calls.
  virtual void setTag(const BRepBuilderGeometryId &id, OdUInt32 tag) = 0;
  //
  // Note! kSetTagMode and kAutoMode are incompatible. You cannot use methods setTag(...) and setInitialTag(...) at the same time.
  // 
