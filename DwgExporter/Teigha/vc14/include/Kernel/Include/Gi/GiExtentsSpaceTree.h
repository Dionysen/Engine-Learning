...
public:
  ...
  OdList<OdGiExtentsSpaceNode<E, O>*>* getPointLeaves(const P& pt, double proximityRadius)
  {
    ...
    nodeProcessPoint(m_pRootNode, pt, proximityRadius);
...
private:
  ...
  void nodeProcessPoint(OdGiExtentsSpaceNode<E, O>* pNode, const P& pt, double r)
  {
    if (pNode == NULL)
      return;

    if (pNode->m_extents.isWithinRange(pt, r))
    {
      if (pNode->isLeave())
      {
        m_theIntersectedLeaves.push_back(pNode);
      ...
      {
        nodeProcessPoint(pNode->m_pLeftChild, pt, r);
        nodeProcessPoint(pNode->m_pRightChild, pt, r);
      }
    }
