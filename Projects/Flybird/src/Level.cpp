...
void Level::Init()
{
    m_TriangleTexture = Texture2D::Create("Projects/Flybird/assets/textures/Triangle.png");
    m_Player.LoadAssets();
...
void Level::OnUpdate(Timestep ts)
{
    ...
    if (m_Player.GetScore() * m_LevelSpeed <= 3)
        m_GameLevel = m_Player.GetScore() * m_LevelSpeed;
    else
        m_GameLevel = 3.0f;
...
void Level::OnImGuiRender()
{
    ...
    ImGui::DragFloat("Level", &m_LevelSpeed, 0.01);
    ImGui::Text("GameLevel: %f", m_GameLevel);
}
...
void Level::CreatePillar(int index, float offset)
{
    ...
    float gap    = 5.0 - m_GameLevel + Random::Float() * 5.0f;
