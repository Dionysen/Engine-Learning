...
class OdGsModelRedirectionWrapper : public Base
{
  ...
  public:
    ...
    */
    virtual void highlight(const OdGiPathNode& path, bool bDoIt = true, const OdGsView* pView = 0)
    {
      for (OdUInt32 nModel = 0; nModel < m_pRedirectionClient->numModels(); nModel++)
        m_pRedirectionClient->gsModelAt(nModel)->highlight(path, bDoIt, pView);
    }
    ...
    /** \details
      ...
      \param pView [in]  Pointer to the view where the specified entity should be highlighted.
    */
    virtual void highlight(const OdGiPathNode& path, const OdGsMarker* pMarkers, OdUInt32 nMarkers, bool bDoIt = true, const OdGsView* pView = 0)
    {
      for (OdUInt32 nModel = 0; nModel < m_pRedirectionClient->numModels(); nModel++)
        m_pRedirectionClient->gsModelAt(nModel)->highlight(path, pMarkers, nMarkers, bDoIt, pView);
    }
    ...
    */
    virtual void hide(const OdGiPathNode& path, bool bDoIt = true, bool bSelectHidden = false, const OdGsView* pView = 0)
    {
      for (OdUInt32 nModel = 0; nModel < m_pRedirectionClient->numModels(); nModel++)
    ...
    /** \details
      ...
      \param pView [in]  Pointer to the view where the specified entity should be hidden.
    */
    virtual void hide(const OdGiPathNode& path, const OdGsMarker* pMarkers, OdUInt32 nMarkers, bool bDoIt = true, bool bSelectHidden = false, const OdGsView* pView = 0)
    {
      for (OdUInt32 nModel = 0; nModel < m_pRedirectionClient->numModels(); nModel++)
