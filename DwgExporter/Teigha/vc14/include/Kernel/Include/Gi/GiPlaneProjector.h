...
public:
  ...
  */
  ...
  /** \details
      Checks whether this conveyor node is enabled.
      \returns 
      true if this conveyor node enabled, false elsewhere.
  */
  virtual bool isEnabled() const = 0;
};

/** \details
  A data type that represents a smart pointer to an <link OdGiXYProjector, OdGiXYProjector> object.
*/
...
public:
  ...
  */
  ...
  /** \details
      Retrieves current projection plane (default projection plane is same with XY plane).
      \returns
      current projection plane as reference to OdGePlane.
  */
  virtual const OdGePlane &plane() const = 0;
  /** \details
      Retrieves current projection direction (default projection direction is same with vector Z-axis).
      \returns
      current projection direction as reference to OdGeVector3d.
  */
  virtual const OdGeVector3d &projectionDirection() const = 0;
};

/** \details
  A data type that represents a smart pointer to an <link OdGiPlaneProjector, OdGiPlaneProjector> object.
*/
