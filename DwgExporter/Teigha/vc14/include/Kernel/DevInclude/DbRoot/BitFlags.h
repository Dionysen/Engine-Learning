...
inline int OdInterlockedSetField(volatile int* dest, int mask, int value)
{
  int lOriginal, lResult;
  do {
    lOriginal = *dest;
    lResult = (lOriginal & (~mask)) | (value & mask);
  } while (OdInterlockedCompareExchange(dest, lResult, lOriginal) != lOriginal);
  return lResult;
}
#endif
...
#ifndef TD_SINGLE_THREAD
  void setFlagsSync(T flags, T mask = kOdDbIdAllBits) { OdInterlockedSetField((volatile int*)&m_flags, (int)mask, (int)flags); }
  T flagsSync(T mask) const { return (T)(OdInterlockedExchangeAdd((volatile int*)(&m_flags), (int)0) & (int)mask); }
#else
  void setFlagsSync(T flags, T mask = kOdDbIdAllBits) { m_flags = (m_flags & (~mask)) | (flags & mask); }
  T flagsSync(T mask) const { return (m_flags & mask); }
#endif
private:
  T m_flags;
};
