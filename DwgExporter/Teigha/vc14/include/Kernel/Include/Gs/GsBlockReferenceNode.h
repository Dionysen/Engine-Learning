...
private:
  ...
  std::unique_ptr<OdSiShape> m_transformedQuery;
};
...
public:
  ...
  enum
  {
    ...
    kForceNotEmpty        = OdGsEntityNode::kLastFlag << 4,
  ...
  bool isForceNotEmpty() const { return GETBIT( m_flags, kForceNotEmpty ); }
  void setForceNotEmpty( bool b ) { SETBIT( m_flags, kForceNotEmpty, b ); }
