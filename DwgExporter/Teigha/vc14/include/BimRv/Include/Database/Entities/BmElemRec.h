...
#ifndef _BM_ELEMREC_H_
#define _BM_ELEMREC_H_
...
class OdBmElemRec;
class OdBmElemRecImpl;

 /** \details
 A data type that represents a smart pointer to an OdBmElemRec object.
 */
typedef OdSmartPtr<OdBmElemRec> OdBmElemRecPtr;

/** \details
 A data type that represents an array of OdBmElemRec objects.
 */
typedef OdArray<OdBmElemRecPtr> OdBmElemRecPtrArray;
...
*/
class TB_DB_EXPORT OdBmElemRec : public OdBmObject {
  ODBM_DECLARE_CUSTOM_CLASS_MEMBERS(OdBmElemRec);
public:
  OdBmObjectId getObjectId() const;
  OdResult setObjectId(const OdBmObjectId&);
