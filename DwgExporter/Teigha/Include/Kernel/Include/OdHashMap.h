...
/* Mostly STL implementations contain hash_map which uses separate hasher and predicate
   ...
   This implementation contatined by all compilers, which uses SGI STL, but
...
class OdHashMapLs : public OdUnorderedMap<Key, Value, _HashFcn, 
#if !defined(__BORLANDC__)
od_std_hash_map_generalized_equal_to<Key, _HashFcn>, 
#endif
Alloc> {};
#elif defined(_MSC_VER)
// MSVC
#if (_MSC_VER >= 1300) // this container available only since vc7
#include <hash_map>
#define HASHMAP_SUPPORTED
...
class OdHashMap : public OdUnorderedMap<Key, Value, HashFcn, 
#if !defined(__BORLANDC__)
std::equal_to<Key>, 
#endif
Alloc> {};
