...
protected:
  ...
  bool checkVisibility(double uScale, double vScale, OdGiViewportDraw* pVd, OdGeExtents2d &resUnCS/*in WCS*/) const;
  ...
  virtual OdResult getImage(ImageCache& pdfInfo, int nPage, OdGiViewportDraw* pVd, const OdDbUnderlayDrawContext& context, OdGeExtents2d& clipRect,
                        double DPI, int ImagePixW, int ImagePixH) const = 0;
...
public:
  ...
  virtual OdGiRasterImagePtr getThumbnail(int nPage, int width = 170, int height = 170) const = 0;
