  ...
  public:
    ...
    */
    virtual bool intersects( const OdGeExtents3d& extents, bool planar = false, const OdGeTol& tol = OdGeContext::gTol) const
    {
      ...
      {
        if (extents.minPoint().x == maxPoint().x || extents.minPoint().y == maxPoint().y
          || minPoint().x == extents.maxPoint().x || minPoint().y == extents.maxPoint().y)
          return false;//extents not intersect, they has one a border for both
